...
// Uses the v1beta generateContent endpoint; key is the AI Studio API key (BYOK).

import type { AppConfig, Conversation } from '@/shared/types'
import { FALLBACK_REPLY } from '@/content/features/auto-reply/prompt'
...
export async function generateReply(
  ...
  if (!config.geminiApiKey) throw new Error('Gemini API key not set')

  const url = `${ENDPOINT}/${encodeURIComponent(config.model)}:generateContent?key=${encodeURIComponent(config.geminiApiKey)}`
